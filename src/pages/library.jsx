...
export default function LibraryPage({ setPage, openVideo, initialQuery }) {
  const data = DATA;
  ...
  // Sync with a query arriving from the nav search bar
  ...
  const filtered = React.useMemo(() => {
    ...
    return list;
  }, [query, filters, sort, data.videos]);
    ...
    <main>
      ...
      <section style={{ paddingTop: 64, paddingBottom: 40, borderBottom: "1px solid var(--border-soft)" }}>
        <div className="container-wide">
          ...
          <h1 style={{ fontSize: "clamp(34px, 7vw, 56px)", marginTop: 16, maxWidth: 800 }}>The reference library<br/>for academic surgery.</h1>
      ...
      <section style={{ paddingTop: 32, paddingBottom: 80 }}>
        <div className="container-wide lib-grid">
          {/* Sidebar */}
          ...
          <div>
            <div className="flex items-center justify-between" style={{ marginBottom: 24, flexWrap: "wrap", gap: 16 }}>
              ...
              <div className="flex gap-12 items-center">
                <select className="select" style={{ height: 36, paddingRight: 32, fontSize: 13, width: "auto" }} value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Sort videos">
                  <option value="newest">Sort: Newest</option>
                  <option value="popular">Sort: Most viewed</option>
                  <option value="duration">Sort: Shortest first</option>
                </select>
                <div style={{ display: "flex", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden" }}>
                  <button onClick={() => setView("grid")} aria-label="Grid view" aria-pressed={view === "grid"} style={{ padding: "8px 12px", background: view === "grid" ? "var(--surface-2)" : "transparent", color: view === "grid" ? "var(--accent)" : "var(--text-3)" }}><Icon.grid/></button>
                  <button onClick={() => setView("list")} aria-label="List view" aria-pressed={view === "list"} style={{ padding: "8px 12px", background: view === "list" ? "var(--surface-2)" : "transparent", color: view === "list" ? "var(--accent)" : "var(--text-3)", borderLeft: "1px solid var(--border)" }}><Icon.list/></button>
                </div>
              </div>
            </div>
...
function CatPill({ label, sub, count, active, onClick }) {
  return (
...
function VideoListRow({ v, onOpen }) {
  return (
    <div className="card listrow" onClick={() => onOpen(v)} role="button" tabIndex={0}
         onKeyDown={(e) => { if (e.key === "Enter") onOpen(v); }} style={{ cursor: "pointer" }}>
