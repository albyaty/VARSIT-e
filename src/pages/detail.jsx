...
import { Icon, VideoCard, ThumbDecor } from "../components.jsx";

export default function DetailPage({ video, setPage, openVideo }) {
  const data = DATA;
  ...
  React.useEffect(() => {
    setTab("overview"); setPlaying(false); setProgress(18); setSaved(false); setNote("");
  }, [v.id]);

  const flash = (msg) => { setNote(msg); window.clearTimeout(flash._t); flash._t = window.setTimeout(() => setNote(""), 2200); };
  const onShare = async () => {
    try { await navigator.clipboard.writeText(window.location.href); flash("Link copied to clipboard"); }
    catch { flash("Couldn't copy — copy from the address bar"); }
  };
  const onCite = async () => {
    const citation = `${v.surgeon}. "${v.title}." VARSIT-E: Expert Surgical Video Education; ${v.year}. ${window.location.href}`;
    try { await navigator.clipboard.writeText(citation); flash("Citation copied"); }
    catch { flash("Couldn't copy citation"); }
  };
  ...
  return (
    <main style={{ paddingBottom: 80 }}>
      ...
      <div style={{ borderBottom: "1px solid var(--border-soft)", padding: "16px 0" }}>
        <div className="container-wide" style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "var(--text-3)", flexWrap: "wrap" }}>
      ...
      <div className="container-wide detail-grid" style={{ paddingTop: 32 }}>
        {/* LEFT */}
        <div>
          {/* Player */}
          <div style={{ position: "relative", aspectRatio: "16/9", borderRadius: 16, overflow: "hidden", background: v.thumb, border: "1px solid var(--border)" }}>
            ...
            {/* Center play */}
            {!(playing && v.embedUrl) && <button onClick={() => setPlaying(!playing)} aria-label={playing ? "Pause" : "Play video"} style={{
            ...
            {/* Bottom HUD */}
            {!(playing && v.embedUrl) && <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: 20, color: "white" }}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-12 items-center">
                  <button style={{ color: "white" }} onClick={() => setPlaying(true)} aria-label="Play">{playing ? "⏸" : "▶"}</button>
          ...
          <div style={{ marginTop: 32 }}>
            ...
            <h1 style={{ fontSize: "clamp(28px, 5vw, 42px)", lineHeight: 1.15 }}>{v.title}</h1>
            <div style={{ marginTop: 20, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
              ...
              <div className="flex gap-8 items-center" style={{ flexWrap: "wrap" }}>
                {note && <span className="text-mono" style={{ fontSize: 12, color: "var(--accent)" }}>{note}</span>}
                <button className="btn btn-secondary" onClick={() => setSaved(s => !s)} aria-pressed={saved}
                  style={saved ? { color: "var(--accent)", borderColor: "var(--accent)" } : undefined}>
                  <Icon.bookmark/> {saved ? "Saved" : "Save"}
                </button>
                <button className="btn btn-secondary" onClick={onShare}><Icon.share/> Share</button>
                <button className="btn btn-secondary" onClick={onCite}><Icon.download/> Cite</button>
              </div>
            </div>
          </div>

          {/* Tabs */}
          <div className="tabbar scroll-hide" style={{ marginTop: 32, borderBottom: "1px solid var(--border-soft)" }}>
            {[
              ["overview", "Overview"],
            ...
            {tab === "steps" && (
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {v.steps.map((s, i) => (
                    <button key={i} className="card" onClick={() => setPlaying(true)} style={{
                      ...
                      <span style={{ width: 28, height: 28, borderRadius: "50%", background: "var(--surface-2)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 600, flexShrink: 0 }}>{i + 1}</span>
        ...
        {/* RIGHT — Sticky meta */}
        <aside className="sticky-side" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div className="container-wide">
            <div className="section-head">
              <div>
                ...
                <h2 style={{ marginTop: 12, fontSize: "clamp(26px, 4vw, 36px)" }}>More from {v.surgeon.split(",")[0]}</h2>
              </div>
            </div>
